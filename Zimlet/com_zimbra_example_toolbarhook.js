...
com_zimbra_example_toolbarhook_HandlerObject = function() {
	...
	param1.value="/zimbra/public/ecoit_plugin50/CAAppletSimple.jnlp";
...
com_zimbra_example_toolbarhook_HandlerObject.prototype._showSelectedMail = function(
	...
	arrayAddress["TO"] = appCtxt.getAppViewMgr().getCurrentView()._msg._addrs.TO;
	arrayAddress["CC"] = appCtxt.getAppViewMgr().getCurrentView()._msg._addrs.CC;
	arrayAddress["BCC"] = appCtxt.getAppViewMgr().getCurrentView()._msg._addrs.BCC;
	
	for (index in arrayAddress)
	for (object in arrayAddress[index].getArray()){
		var obj = arrayAddress[index].getArray()[object];
		var check = true;
		jQuery.ajax(
				{
					url:"/zimbra/public/ajax/checkPublicKeyExist.jsp",
					data:{uid:obj.address},
					async: false,
					success:function(data){
		...
		if (check==false){
			alert("Lỗi: Không tìm thấy chứng thực cho địa chỉ email "+obj.address);
			return false;
		}
	}
...
com_zimbra_example_toolbarhook_HandlerObject.prototype._showSelectedMailSign = function(
	...
	var arrayAddress= new Array();
	arrayAddress["TO"]= curentView._msg._addrs.TO;
	arrayAddress["CC"]= curentView._msg._addrs.CC;
	arrayAddress["BCC"]= curentView._msg._addrs.BCC;
